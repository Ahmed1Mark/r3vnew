...
if (interaction.customId === 'close_ticket') {
    ...
    try {
       ...
       await interaction.reply({ content: `> **Ticket Closed by : ${member}**`, ephemeral: true });
    } catch (error) {
    ...
    try {
        ...
        // هنا لن نرسل المحادثة إلى الرسائل المباشرة
        // بدلاً من ذلك، سنقوم بإنشاء ملف PDF وإرساله في القناة نفسها
        const pdfBuffer = await createPDF(transcript);
        await ticketChannel.send({ files: [{ attachment: pdfBuffer, name: 'transcript.pdf' }] });
        await interaction.reply({ content: 'Transcript has been created and sent to the ticket channel.', ephemeral: true });
    } catch (error) {
...
async function createPDF(transcript) {
    try {
        ...
        let cursorY = textHeight;

        for (const textLine of textLines) {
            if (cursorY < 50) {
                await pdfDoc.addPage([500, 700]);
                cursorY = textHeight;
            }
            const text = page.drawText(textLine, { x: 50, y: cursorY, size: textSize, font: helveticaFont, color: rgb(0, 0, 0) });
            cursorY -= textSize * 1.2;
        }
